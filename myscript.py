...
def ProcessOutputHmmscan(input_path,output_path):
    #process the output files generated by hmmscan
...
def CreateSet(input_path,output_path):
    # generates the domain composition of each species in tree file and write in a file
    files = os.listdir(input_path)
    for file in files:
...
def CreateCombineSet(input_path,output_path):
    # generates the domain combination composition of each species in tree file and write in a file
    files = os.listdir(input_path)
    for file in files:
        name = re.search('(.*)\.',file).group(1)
        set1 = set()
...
def AncestralContent(species,tree):
    # caculate gain and lose of each node in tree 
    AncestralNode = {}
...
def FormLineageSet(dataset):
    # load domain set in each lineage and return dictionary and set
    all_domain = set()
    lineage = {}
...
def DmtbloutInit(dmtblout_path,name=0):
    #process the output files generated by hmmscan (format = file.dmtblout)
...
def LineageSet(file_lists,path):
    # read domain composition file and load all domain in each taxon .
    # return a set
    set_list = []
    for i in file_lists:
        locals()[str(i)] = set()
...
def EverySet(file_lists,path):
    # read domain composition file and load all domain in each species .
    set_list = []
    for i in file_lists:
        locals()[str(i)] = set()
        with open(path+i,'r') as f:
...
def CoreSet(set1,everyset):
    # generates core domain/combination set
    core = set()
    for domain in set1:
        num = 0
        for s in everyset:
...
if __name__ == '__main__':
    ...
    if os.path.exists('./examples/input/hmmscan_output'):
        shutil.rmtree('./examples/input/hmmscan_output')
    os.mkdir('./examples/input/hmmscan_output')

    if os.path.exists('./examples/input/domain_set'):
        shutil.rmtree('./examples/input/domain_set')
    os.mkdir('./examples/input/domain_set')

    if os.path.exists('./examples/input/domain_coreset'):
        shutil.rmtree('./examples/input/domain_coreset')
    os.mkdir('./examples/input/domain_coreset')

    if '.DS_Store' in hmmscan_files:
        hmmscan_files.remove('.DS_Store')
